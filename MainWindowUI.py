...
class MainWindowUI(QMainWindow):
    def __init__(self):
        ...
        ui_file = os.path.join(current_dir, "design.ui")
    ...
    def _bind_events(self):
        ...
        pub.subscribe(self.handle_edge_detection, "edges detected")

    def handle_edge_detection(self, result):
        self.images.output1 = result["x_edges"]
        self.images.output2 = result["y_edges"]
        self.update_display()
    ...
    def upload_image1(self):
        ...
        self.images.image1 = self.image
        piximage = QPixmap.fromImage(self.image.qimg.scaled(450,650))
    ...
    def upload_image2(self):
        ...
        piximage = QPixmap.fromImage(image.qimg.scaled(450,650))
    ...
    def update_display(self):
        if self.ui.mixerModeGroup.isChecked():
            ...
            self.show_widget(self.ui.out1widget)
            self.ui.outputOneLabel.setText("Horizontal Edge")
            self.ui.originalImageLabel.setText("Image")
            self.show_widget(self.ui.out2widget)
            self.ui.outputTwoLabel.setText("Vertical Edge")
        ...
        if self.images.output1 and not self.ui.mixerModeGroup.isChecked():
            piximage = QPixmap.fromImage(self.images.output1.qimg.scaled(450,650))
            self.ui.OutputImage1Label.setPixmap(piximage)
            self.ui.OutputImage1Label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        if self.images.output2 and self.ui.mixerModeGroup.isChecked():
            piximage = QPixmap.fromImage(self.images.output2.qimg.scaled(450,650))
    ...
    def select_mode(self,index):
        if index == 1:
            ...
            self.ui.otherModesGroup.setChecked(False)
            print("Lets Detect edges")
            pub.sendMessage("detect edges", image=self.image.image_data, filter_type=self.filterTypeComboBox.currentText())
        else:
            self.ui.mixerModeGroup.setChecked(False)
            self.ui.noiseModeGroup.setChecked(False)
